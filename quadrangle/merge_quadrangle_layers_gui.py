...
def gcodeMerging(a, b, c, d, degree, width, shell_width, circle, z0, height_each, height_sum):
    baseGcode = fileRead()
...
def button_clicked_event():
    ...
    ax = float(mainwindow_ui.lineEdit_4.text())
    bx = float(mainwindow_ui.lineEdit_5.text())
    cx = float(mainwindow_ui.lineEdit_7.text())
    dx = float(mainwindow_ui.lineEdit_8.text())
    ay = float(mainwindow_ui.lineEdit_11.text())
    by = float(mainwindow_ui.lineEdit_10.text())
    cy = float(mainwindow_ui.lineEdit_9.text())
    dy = float(mainwindow_ui.lineEdit_6.text())
    degree = float(mainwindow_ui.lineEdit.text())
    width = float(mainwindow_ui.lineEdit_2.text())
    shell_width = float(mainwindow_ui.lineEdit_3.text())
    circle = mainwindow_ui.spinBox.value()
    z0 = 0.2
    height_each = float(mainwindow_ui.lineEdit_12.text())
    height_sum = float(mainwindow_ui.lineEdit_13.text())
    a = (ax, ay)
    b = (bx, by)
    c = (cx, cy)
    d = (dx, dy)
    ...
    finalGcode = gcodeMerging(a, b, c, d, degree, width, shell_width, circle, z0, height_each, height_sum)
...
if __name__ == '__main__':
    ...
    mainwindow_ui = ui.Ui_Form()
    qMainWindow = QMainWindow()
    mainwindow_ui.setupUi(qMainWindow)
    mainwindow_ui.pushButton.clicked.connect(button_clicked_event)
